...
const Login = () => {
    ...
    const { login, registrar } = useContext(UserContext);
    ...
    const onSubmit = async (data) => {
        ...
        switch (opcion) {
            ...
            case "signUp":
                try {
                    await registrar(data.mail, data.password);
                    navegacion("/")
                } catch (error) {
                    toast({
            ...
            <div className="bg-slate-50 drop-shadow-[0_0px_8px_rgba(0,0,0,0.10)] dark:drop-shadow-[0_0px_5px_white] px-14 py-7 rounded-2xl w-[80%] flex flex-col gap-4 sxm:w-[50%] lg:w-[35%]">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className=" flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <button type="submit" onClick={() => setOpcion("signInGoogle")} className="border-bunker-300 tracking-[1px] text-bunker-500 bg-bunker-100 hover:bg-bunker-900 hover:text-bunker-50 border-[1px]  p-2 rounded-md transition-colors duration-300 ease-in-out flex justify-center items-center gap-2">Sign In with
